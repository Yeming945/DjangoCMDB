...
class Asset(models.Model):
    ...
    manufacturer = models.ForeignKey(
        'ManuFacturer', null=True, blank=True, verbose_name='制造商', on_delete=models.SET_NULL)
...
class Sever(models.Model):
    ...
    raid_type = models.CharField(
        max_length=512, null=True, blank=True, verbose_name='Raid类型')

    os_type = models.CharField(max_length=64, null=True,
                              blank=True, verbose_name='操作系统类型')
    os_distribution = models.CharField(
        max_length=64, null=True, blank=True, verbose_name='发行商')
    os_release = models.CharField(
        max_length=64, null=True, blank=True, verbose_name='操作系统版本')

    def __str__(self):
        return '%s--%s--%s <sn:%s>' % (self.asset.name, self.get_sub_asset_type_dispaly(), self.model, self.asset.sn)
...
class SecurityDevice(models.Model):
    ...
    def __str__(self):
        return self.asset.name + '--' + self.get_sub_asset_type_dispaly() + str(self.model) + " id:%s " % self.id
...
class NetworkDevice(models.Model):
    ...
    def __str__(self):
        return '%s--%s--%s <sn:%s>' % (self.asset.name, self.get_sub_asset_type_dispaly(), self.modelm, self.asset.sn)
...
class Software(models.Model):
    ...
    version = models.CharField(
        max_length=64, unique=True, help_text='例如: RedHat relate 7 (Final)', verbose_name='软件/系统版本')
...
class CPU(models.Model):
    ...
    cpu_thread_count = models.PositiveSmallIntegerField(
        default=1, verbose_name='CPU线程数')
    cpu_frequency = models.DecimalField(
        max_digits=3, decimal_places=2, verbose_name='CPU主频(GHZ)')
...
class RAM(models.Model):
    ...
    def __str__(self):
        return '%s: %s :%s :%s :%s' % (self.asset.name, self.model, self.slot, self.capacity, self.ram_frequency)
...
class Disk(models.Model):
    ...
    manufacturer = models.CharField(
        '磁盘制造商', max_length=64, blank=True, null=True)
    ...
    disk_protocol = models.CharField(
        '磁盘协议', max_length=16, choices=disk_protocol_choice, default='SATA')

    def __str__(self):
        return '%s: %s: %s: %sGB: %s ' % (self.asset.name, self.model, self.slot, self.capacity)
...
class NIC(models.Model):
    ...
    manufacturer = models.CharField(
        '网卡制造商', max_length=64, blank=True, null=True)
...
class Manufacturer(models.Model):
    ...
    telephone = models.CharField(
        max_length=30, blank=True, null=True, verbose_name='支持电话')
    memo = models.CharField(max_length=128, blank=True,
                            null=True, verbose_name='备注')
...
class BusinessUnit(models.Model):
    ...
    parent_unit = models.ForeignKey(
        'self', blank=True, null=True, related_name='parent_level', on_delete=models.SET_NULL)
    telephone = models.CharField(
        max_length=30, blank=True, null=True, verbose_name='业务线')
    memo = models.CharField(max_length=128, blank=True,
                            null=True, verbose_name='备注')
...
class Contract(models.Model):
    ...
    price = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name='合同金额')
    ...
    license_num = models.IntegerField(
        blank=True, null=True, verbose_name='license数量')
...
class EventLog(models.Model):
    ...
    event_type = models.SmallIntegerField(
        '时间类型', choices=event_type_choice, default=4)
...
class NewAssetApprovalZone(models.Model):
    ...
    asset_type = models.CharField(choices=asset_type_choice, default='server',
                                  max_length=64, blank=True, null=True, verbose_name='资产类型')
    manufacturer = models.CharField(
    ...
    cup_model = models.CharField(
        max_length=128, blank=True, null=True, verbose_name='CPU型号')
    cpu_count = models.PositiveSmallIntegerField(
        blank=True, null=True, verbose_name='CPU物理数量')
    cpu_core_count = models.PositiveSmallIntegerField(
        blank=True, null=True, verbose_name='CPU核心数量')
    os_distribution = models.CharField(
